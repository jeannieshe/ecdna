...
arg_mapper <- function(celltag.version) {
  ...
  celltag.df <- data.frame(version = c("8N-v1", "8N-v2", "8N-v3","multi-v1","custom"), #modified to include Wennerberg Lab custom CellTags
                           grep.pattern = c("GGT[ACTG]{8}GAATTC",
                                            "GTGATG[ACTG]{8}GAATTC",
                                            "TGTACG[ACTG]{8}GAATTC",
                                            "[ACTG]{3}GT[ACTG]{3}CT[ACTG]{3}AG[ACTG]{3}TG[ACTG]{3}CA[ACTG]{3}",
                                            "[AT]{2}[ACGT]{4}[AT]{2}[ACGT]{4}[AT]{2}"),
                           ct.len = c(17,17,17,28,14),
                           sec.pattern = c("CAGCTTCCGAG", "CAGCTTCCGAG", "CAGCTTCCGAG","CCGGTAA","GAATTCG"),
                           stringsAsFactors = F)
